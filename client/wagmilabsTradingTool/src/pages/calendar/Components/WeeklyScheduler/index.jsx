  ...
  ({
    ...
    const getUsedHoursBlocks = () => (
      allEventsInAllWeakdays.map((eventsInWeekDay) => (
        eventsInWeekDay.map((el)=>moment(el?.timestamp).hours()).filter((value, index, array) => array.indexOf(value) === index)).map((el)=>({hour:el}))
      )
    )
    const [usedHoursBlocks, setUsedHoursBlocks] = useState(getUsedHoursBlocks());
    const onSelectDate = (date) =>{
      showSelectedDate(date);
      setUsedHoursBlocks((prev) => prev.map((el, index)=> {
        ...
        <div className="weekday-calendar-column">
          <div className="weekday-calendar-inner-column">
            ...
            {hoursIntervals.map((h)=>(
              <div className={usedHoursBlocks[dayIndex]
                              && usedHoursBlocks[dayIndex]?.map((el)=>el?.hour)
                              .includes(h.idx) ? activeHourClass(usedHoursBlocks[dayIndex].find((el)=>el.hour === h.idx)) : "hour-section"}
              >
                {getEventsInWeekday(dayIndex).map((event)=>(
                  <>
                  {moment(event?.timestamp)
                  .hours() === h.idx
                    && <div className="hour-slot" onClick={()=> onSelectDate(moment(event?.timestamp)?.toDate())}>
                        {event?.spaceName}
                      </div>
                  }
                  </>

...
const BubbleChart = memo(({ totalListings, totalSales, floorPrice }) => {
    ...
    const minTimestamp = useMemo(() => Math.min(...totalListings?.map(listing => listing.timestamp * 1000), ...totalSales?.map(sale => sale.timestamp)), [totalListings, totalSales])
    const maxTimestamp = useMemo(() => Math.max(...totalListings?.map(listing => listing.timestamp * 1000), ...totalSales?.map(sale => sale.timestamp)), [totalListings, totalSales])

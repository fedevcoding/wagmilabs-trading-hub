...
const Multiplechart = () => {
  ...
  const [chartsTypes, setChartsTypes] = useState({ apiEndOne: "floorPrice", apiEndTwo: "owners" })
  ...
  useEffect(() => {
    async function chart() {
      try {
        let chartsOneData = await fetch(`${baseUrl}/api/v1/wagmilabs/${params.id}/${apiEndUrl[chartsTypes.apiEndOne]}`, {
          headers: {
        ...
        let chartsTwoData = await fetch(`${baseUrl}/api/v1/wagmilabs/${params.id}/${apiEndUrl[chartsTypes.apiEndTwo]}`, {
          headers: {
            "x-auth-token": localStorage.jsonwebtoken
          },
        })
        ...
        console.error(err)
        document.querySelector(".loadingChart").innerText = "Unable to load data, try again later."
      }
  ...
  useEffect(() => {
    ...
    if (stats) {
      ...
      usableStats.chartsOneData.forEach(stat => {
        ...
        dates.push(stat.date_time_stamp.toLocaleTimeString('en-GB', options).replace(" at", "") + ":00")
      });
  ...
  function redrawPeriod(e) {
    if (stats) {
      ...
      var seventhDay = new Date(new Date().getTime() - (times[e] * 60 * 60 * 1000));
      ...
      let canvas = document.createElement('canvas')
      canvas.classList.add("multiple-chart");
      let container = document.querySelector(".multiple-containerChart").appendChild(canvas)
      setNewStats({ chartsOneData: filteredChartOneData, chartsTwoData: filteredChartTwoData })
    }
  ...
  return (
    <>
      <div className='chartSelectors'>
        <div>
          <p>Chart one: </p>
          <select name="chartType" className='chartOneSelect' onChange={redrawTypeCharts}>
        ...
        <div>
          <p>Chart two: </p>
          <select name="chartType" className='chartTwoSelect' onChange={redrawTypeCharts}>
            <option value="floorPrice">Floor price</option>
            <option value="volume">Volume</option>
            <option value="sales">Sales</option>
            <option selected value="owners">Owners</option>
            <option value="averagePrice">Average price</option>
            <option value="activeListings">Active listings</option>
            <option value="nftsPerOwner">Nfts per owner</option>
            <option value="marketCap">Market cap</option>
        ...
        <div>
          <p>Chart period: </p>
          <select name="period" className='chartPeriodSelect' onChange={e => redrawPeriod(e.target.value)}>
      ...
      <div className='multiple-containerChart'>
        {loading && <p className='loadingChart'>Loading... <svg className="spinner" width="65px" height="65px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg"><circle className="path" fill="none" strokeWidth="6" strokeLinecap="round" cx="33" cy="33" r="30"></circle></svg></p>}

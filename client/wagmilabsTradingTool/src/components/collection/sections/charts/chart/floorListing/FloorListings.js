  ...
  useEffect(()=>{
    async function chart(){
      try{
        ...
        setLoading(true)
        let listingsData = await fetch(`${baseUrl}/api/v1/wagmilabs/${params.id}/listed_count`, {
          headers: {
        ...
        let floorPriceData = await fetch(`${baseUrl}/api/v1/wagmilabs/${params.id}/floor_price`, {
          headers: {
              "x-auth-token": localStorage.jsonwebtoken
          },
      })
...
useEffect(()=>{
  ...
  if(stats){
      ...
      usableStats.floorPriceData.forEach(price => {
          ...
          dates.push(price.date_time_stamp.toLocaleTimeString('en-GB', options).replace(" at", "") + ":00")
      });
...
function redrawPeriod(e){
  if(stats){
      ...
      let container = document.querySelector(".floor-listing-containerChart").appendChild(canvas)
      setNewStats({listingsData: filteredListings, floorPriceData: filteredFloorPrice})
  }
  ...
  return (
    <>
        ...
        <div>
            <p>Chart period: </p>
            <select name="period" className='chartTypeSelect' onChange={e => redrawPeriod(e.target.value)}>

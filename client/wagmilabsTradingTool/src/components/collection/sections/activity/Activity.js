...
const Activity = ({address}) => {
  ...
  const [collectionActivityLoading, setCollectionActivityLoading] = useState(true)
  ...
  async function getChartData(){
    ...
    const averagePrices = data.map(item => roundPrice2(item.averageprice)).reverse()
    const volumes = data.map(item => roundPrice2(item.volume)).reverse()
    const days = data.map(item => moment(item.day).format("DD/MM/YYYY")).reverse()
  ...
  async function getActivityData(){
    ...
    let data = await fetch(`${baseUrl}/collectionActivity/${address}?types=${filters}`, {
      headers: {
        "x-auth-token": localStorage.jsonwebtoken
      }
    })
      ...
      <tr key={key} className={`collection-activity-single-container ${isLast && "last-token"}`}>
        ...
        <td className="collection-activity-single-type">
          <div className='collection-activity-marketplace-container'>
            {
              marketplaceImage ?
              <img src={marketplaceImage} className="collection-activity-marketplace-image"></img>
              :
    ...
    <div className='collection-activity-section'>
      ...
      <div className='collection-activity-filters-container'>
        ...
        <div className='collection-activity-filters-categories'>
          <div onClick={() => changeActivityFilter("sale")} className={`${collectuonActivityFilter.includes("sale") ? "active" : ""}`}>
            <i className="fa-light fa-bag-shopping"></i>
            Sales
          </div>
          <div onClick={() => changeActivityFilter("ask")} className={`${collectuonActivityFilter.includes("ask") ? "active" : ""}`}>
            <i className="fa-light fa-tag"></i>
            Listings
          </div>
        </div>
        <div className='collection-activity-filters-categories'>
          <div onClick={() => changeActivityFilter("bid")} className={`${collectuonActivityFilter.includes("bid") ? "active" : ""}`}>
            <i className="fa-light fa-megaphone"></i>
            Offers
          </div>
          <div onClick={() => changeActivityFilter("transfer")} className={`${collectuonActivityFilter.includes("transfer") ? "active" : ""}`}>

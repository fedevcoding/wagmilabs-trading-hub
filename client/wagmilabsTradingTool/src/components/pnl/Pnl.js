  ...
  useEffect(()=>{
    ...
    async function getTradeItems(){
      ...
      for(let i = 0; i < tokenIdsContainer.length; i++){
        ...
        // };
        let data = await fetch(`https://api.opensea.io/api/v1/assets?order_direction=asc&limit=50${tokenIds}${contractAddresses}`)
        data = await data.json()

        data.assets.sort((a, b)  => {
          return sorterContainer[i].indexOf(a.token_id) - sorterContainer[i].indexOf(b.token_id)
        })
      ...
      data.forEach(trade => {
        ...
        totalTradeItems.forEach(openseaTrade => {
          ...
          if(contractAddress == openseaTrade?.asset_contract?.address && tokenId == openseaTrade.token_id){
  ...
  return (
    <div style={{color: "white"}}>
      <table>
        ...
        <tbody>
          {tradesItems && tradesItems.map(item => {

            const {name, image, buyPrice, sellPrice, gas, flipPnl, approvalFee} = item

            return(
              <tr>
                <td><img src={image} className="pnl-img" />{name}</td>

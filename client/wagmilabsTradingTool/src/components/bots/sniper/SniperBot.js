...
const SniperBot = () => {
  ...
  const {snipingTasks, setSnipingTasks, currentSnipingTasks} = useContext(UserDataContext)
  ...
  const sniperTasksMapping = useMemo(()=> snipingTasks.map((task, index) => {

    const {address, maxFeePerGas, maxPriorityFeePerGas, maxItems, accountAddress, id, status} = task
    const {min, max} = task.price
    ...
    return(
      <tr className='sniper-bot-single-task' key={index}>
        ...
        <td className='sniper-bot-cancel-task'>
          <Button colorScheme={"red"} onClick={() => removeTask(address, id)}>Cancel</Button>
        </td>
      </tr>
    )
  }), [snipingTasks])
    ...
    <section className='sniper-bot-section'>
        ...
        <div className='sniper-bot-sections'>
          <div className='sniper-bot-sections-left'>
            <button className='sniper-bot-sections-buttons active'>Active tasks</button>
            <button className='sniper-bot-sections-buttons'>Activity</button>
          </div>

          <div className='sniper-bot-option-buttons'>
                <Button colorScheme='blue' onClick={newTaskModal}>New task</Button>
        ...
        <NewTaskModal setOpenNewTask={setOpenNewTask} openNewTask={openNewTask} closeTaskModal={closeTaskModal} setSnipingTasks={setSnipingTasks} snipingTasks={snipingTasks} />
    </section>
  )
}

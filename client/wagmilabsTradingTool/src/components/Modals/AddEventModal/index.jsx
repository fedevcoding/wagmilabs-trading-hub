...
export const AddEventModal = React.memo(({ isOpen, onClose, onSave, section, selectedDate, isAdmin }) => {
  ...
  const addEvent = () => {
    if (!eventInfo.date || !eventInfo.name || (section === "drops" && (!eventInfo.price || !eventInfo.supply))) {
      onOpenAlert();
    } else {
  ...
  const handleAddLinkClick = () => {
    if (inputLinkList.length < 3) setInputLinkList([...inputLinkList, { firstName: "", lastName: "" }]);
  };
        ...
        <ModalBody className="modal-body">
          ...
          {isAdmin && (section === "events" || section === "spaces") && (
            <div className="field-container">
              More
              <NumberInput>
                <NumberInputField placeholder="More" color={"white"} onChange={e => updateEventInfo(e, "more", true)} />
              </NumberInput>
            </div>
          )}
          <div className="field-container">
            Name *
            <HStack>
              <Input placeholder="Name" color={"white"} onChange={e => updateEventInfo(e, "name", true)} />
            </HStack>
          </div>
          {section === "spaces" && (
            <div className="field-container">
              Host
              <HStack>
                <Input placeholder="Host" color={"white"} onChange={e => updateEventInfo(e, "spaceHost", true)} />
              </HStack>
            </div>
          )}
          <div className="field-container">
            ...
            <HStack>
              <div className="input-links-list">
                {inputLinkList.map((_, i) => (
                  <div className="input-link-container" key={JSON.stringify(_)}>

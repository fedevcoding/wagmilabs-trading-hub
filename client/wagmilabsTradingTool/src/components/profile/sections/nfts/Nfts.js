    ...
    <>
      {
        ...
        <SmartListModal quickListData={quickListData} listingSettings={listingSettings} listNft={listNft} closeSmartListingModal={closeSmartListingModal} ethPrice={cryptoPrices?.ethPrice} setQuickListData={setQuickListData} toast={toast} />
      }
...
const SmartListModal = ({ quickListData, listingSettings, listNft, closeSmartListingModal, ethPrice, setQuickListData, toast }) => {
  ...
  async function getTokenData() {
    ...
    try {
      ...
      let res = await fetch(`${baseUrl}/tokenListPrice?tokenId=${tokenId}&contractAddress=${contractAddress}&marketplace=${marketplace}&listingType=${listingType}&ethPrice=${ethPrice}`, {
        headers: {
          "x-auth-token": localStorage.jsonwebtoken
        }
      })

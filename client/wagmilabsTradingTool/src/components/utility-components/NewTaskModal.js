import { Button, HStack, Input, NumberInput, NumberInputField, Radio, RadioGroup, Select, Stack } from '@chakra-ui/react'
...
const NewTaskModal = ({openNewTask, closeTaskModal, setSnipingTasks, snipingTasks, currentSnipingTasks}) => {
    ...
    function updateTaskInfo(e, type, isEvent){
        ...
        switch(type){
            case "address":
                ...
                break
            case "min":
                setTaskInfo(prev => ({...prev, price: {...prev.price, min: value}}))
                break
            case "max":
                setTaskInfo(prev => ({...prev, price: {...prev.price, max: value}}))
                break
            case "privateKey":
                ...
                <div className='new-task-modal-wrapper'>
                    ...
                    <div>
...

                        {
                            walletType === "key" ?
                            <HStack>
                                <Input placeholder='Private key' color={"white"} onChange={e => updateTaskInfo(e, "privateKey", true)} />
                            </HStack>
                            :
                    ...
                    <div>
                        Max Fee Per Gas
                        <HStack>
                            <Input placeholder='1.5' color={"white"} onChange={e => updateTaskInfo(e, "maxFeeGas", true)} defaultValue={defaultMaxFeePerGas}/>
                        </HStack>

                        Max Priority Fee Per Gas
                        <HStack>
                            <Input placeholder='30' color={"white"} onChange={e => updateTaskInfo(e, "maxPriorityFeeGas", true)} defaultValue={defaultMaxPriorityFeePerGas}/>
                        </HStack>
                    </div>
                    ...
                    <div>
                        ...
                        <NumberInput>
                            <HStack>
                                <NumberInputField placeholder='N. items' color={"white"} onChange={e => updateTaskInfo(e, "maxItems", true)}/>
...
const WalletMappings = ({updateTaskInfo})=> {
    ...
    return (
        <>
            {
                useMemo(()=> wallets?.map(wallet => {
                        return(
                            <option key={crypto.randomUUID()} value={wallet.key} style={{background: "black"}}>{wallet.name}</option>
                        )
                }), [])
            }
        </>

...
export const useTransferItem = () => {
    ...
    async function transferNft(contract, tokenId, toAddress) {
        try {
            ...
            const contractInstance = new ethers.Contract(contract, safeTransferAbi, signer);
            contractInstance.connect(signer)
            const fromAddress = await signer.getAddress();
            const tx = await contractInstance["safeTransferFrom(address,address,uint256)"](fromAddress, toAddress, tokenId);
            await tx.wait(1);
